...
{
    public static UnitsUIManager Instance;
    ...
    public void DropUnit(Defines.UnitType trooptype)
    {
        int slotcost = 0;
        foreach (Transform unititem in m_SelectedUnitParent)
        {
            if (unititem.GetComponent<UnitUIItem>() && unititem.GetComponent<UnitUIItem>()._UnitType == trooptype)
            {
    ...
    // while select Units
    public void AddUnit(Defines.UnitType trooptype)
    {
        int slotcost = 0;
        foreach (Transform unititem in m_SelectedUnitParent)
        {
